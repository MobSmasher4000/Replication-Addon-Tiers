...
import org.mob.replication_addon_tiers.block.custom.matterTank.MatterTankTier6BlockEntity;
...
public class MatterTankTier6ComponentProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor> {
    ...
    public void appendTooltip(ITooltip iTooltip, BlockAccessor blockAccessor, IPluginConfig iPluginConfig) {
        if (blockAccessor.getServerData().contains("MatterStack")) {
            ...
            iTooltip.add(new ProgressElement((float)(matterStack.getAmount() / (double) ReplicationConfig.MatterTank.CAPACITY * Config.tankTier6), matterStack.isEmpty() ? Component.translatable("tooltip.titanium.tank.empty") : Component.translatable(matterStack.getTranslationKey()).append(" ").append(NumberUtils.getFormatedBigNumber(matterStack.getAmount())), IElementHelper.get().progressStyle().color(color.getRGB()).textColor(16777215), BoxStyle.getNestedBox(), false));
        }
    ...
    public void appendServerData(CompoundTag compoundTag, BlockAccessor blockAccessor) {
        MatterTankTier6BlockEntity blockEntity = (MatterTankTier6BlockEntity) blockAccessor.getBlockEntity();
        compoundTag.put("MatterStack", ((IMatterTank)blockEntity.getTanks().get(0)).getMatter().writeToNBT(new CompoundTag()));
    }

    static {
        MATTER_TANK_LOCATION = ResourceLocation.fromNamespaceAndPath(ReplicationAddonTiers.MOD_ID, "matter_tank_tier_6");
    }
}
